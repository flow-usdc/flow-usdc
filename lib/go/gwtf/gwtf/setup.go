...
func NewGoWithTheFlowError(fileName string) (*GoWithTheFlow, error) {
	...
	for account, key := range config.EmulatorAccounts {
        fmt.Println(fmt.Sprintf("emulator account %s, %s", account, key))
	...
	for name, rawAccount := range rawAccounts {
        ...
        fmt.Println(fmt.Sprintf("raw account addr %s", rawAccount.Address))
        fmt.Println(fmt.Sprintf("raw account keys %s", rawAccount.Keys))
